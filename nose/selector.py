...
class Selector:
    ...
    def matches(self, name):
        ...
        return (
            (
                ...
                or (
                    self.include
                    and any(inc.search(name) for inc in self.include)
                )
            )
            and (
                not self.exclude
                or not any(exc.search(name) for exc in self.exclude)
            )
        )
    ...
    def wantClass(self, cls):
        ...
        if plug_wants is not None:
            log.debug(
                "Plugin setting selection of %s to %s",
                cls,
                plug_wants,
            )
            wanted = plug_wants
    ...
    def wantDirectory(self, dirname):
        ...
        if ispackage(dirname):
            wanted = (
                not self.exclude
                or not any(exc.search(tail) for exc in self.exclude)
            )
        else:
            wanted = (
                self.matches(tail)
    ...
    def wantFile(self, file):
        ...
        if ignore_matches:
            log.debug(
                '%s matches ignoreFiles pattern; skipped',
                base,
            )
            return False
    ...
    def __init__(self, name, workingDir=None):
        ...
        if self.filename:
            ...
            if not op_isabs(self.filename):
                self.filename = op_abspath(
                    op_join(workingDir, self.filename)
                )
