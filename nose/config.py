...
filterwarnings("always", category=DeprecationWarning, module=r"(.*\.)?nose\.config")
...
class ConfiguredDefaultsOptionParser:
    ...
    def _readFromFilenames(self, filenames):
        ...
        for filename in filenames:
            ...
            try:
                ...
                raise ConfigError(
                    "Error reading config file {!r}: {}".format(filename, str(exc))
                )
    ...
    def _readFromFileObject(self, fh):
        ...
        try:
            ...
            raise ConfigError("Error reading config file {!r}: {}".format(filename, str(exc)))
    ...
    def _applyConfigurationToValues(self, parser, config, values):
        for name, value, filename in config:
            ...
            try:
                ...
                self._file_error(
                    "Error reading config file %r: " "%s" % (filename, msg),
                    name=name,
                    filename=filename,
                )
...
class Config:
    ...
    def __repr__(self):
        ...
        keys.sort()
        return "Config(%s)" % ", ".join(["{}={!r}".format(k, d[k]) for k in keys])

    __str__ = __repr__

    def _parseArgs(self, argv, cfg_files):
        def warn_sometimes(msg, name=None, filename=None):
            if hasattr(self.plugins, "excludedOption") and self.plugins.excludedOption(
                name
            ):
                msg = (
                    "Option %r in config file %r ignored: "
                    "excluded by runtime environment" % (name, filename)
                )
    ...
    def configureWhere(self, where):
        ...
        for path in where:
            if not self.workingDir:
                ...
                if abs_path is None:
                    raise ValueError(
                        "Working directory '%s' not found, or " "not a directory" % path
                    )
                log.info("Set working dir to %s", abs_path)
                self.workingDir = abs_path
                if self.addPaths and os.path.exists(
    ...
    def getParser(self, doc=None):
        ...
        parser.add_option(
            ...
            metavar="FILE",
            help="Log debug messages to this file " "(default: sys.stderr)",
        )
        parser.add_option(
            "--logging-config",
            "--log-config",
        ...
        parser.add_option(
            ...
            dest="exclude",
            metavar="REGEX",
            help="Don't run tests that match regular " "expression [NOSE_EXCLUDE]",
        )
        parser.add_option(
            "-i",
            "--include",
        ...
        parser.add_option(
            "--traverse-namespace",
            action="store_true",
            default=self.traverseNamespace,
            dest="traverseNamespace",
...
def user_config_files():
    ...
    return list(filter(os.path.exists, map(os.path.expanduser, config_files)))
